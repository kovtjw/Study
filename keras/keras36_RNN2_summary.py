...
#2. 모델구성
model = Sequential()
model.add(SimpleRNN(20,activation='relu',input_shape = (3,1))) # input_shape 할 때 행은 넣지 않는다. 
model.add(Dense(32, activation='relu'))       # 모든 activation은 다음으로 보내는 값을 한정 시키기 때문에 윗 줄에서도 가능하다.
model.add(Dense(16))
model.add(Dense(8))
model.add(Dense(1))
...
10*10= 100 + 10 + 10
dense 넘어가면, unit 개수 * dense 값 + bias
'''
